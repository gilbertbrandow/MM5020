...
\[
h(X) = a_0 + a_1 X + \cdots + a_n X^n, \quad \text{with } a_0 = 1, \ a_i \in \mathbb{Z}.
\]
Then define \( f(X) := a_1 + a_2 X + \cdots + a_n X^{n-1} \in \mathbb{Z}[X] \), so that
\[
h(X) = 1 + X f(X),
\]
...
\]
This follows from the fact that \( u(X) \equiv 1 \mod X \), so it is invertible modulo \( X^{r+1} \), and therefore:
\[
S_r \equiv u(X) a_r(X) \mod X^{r+1} \quad \text{for some } a_r(X),
\]
...
Because \( b_r(X) \in \mathbb{Z}[X] \), the set \( \{ b_r(X) \mod \delta \} \) is finite modulo \( \delta \), so there exist integers \( r < s \) such that:
\[
b_s(X) \equiv b_r(X) \mod \delta \quad \Rightarrow \quad b_s(X) - b_r(X) \in (\delta) \subset I.
\]
It follows that:
\[
